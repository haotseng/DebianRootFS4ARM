#!/bin/bash
#
# build your own ARM64-Debian Root-FS image.
#
# you need at least
...
#
# Debian 8 use systemd instead of init, so we don't need the file /etc/inittab
#
...
rm -f /debconf.set
apt-get update
apt-get -y install git-core binutils ca-certificates initramfs-tools uboot-mkimage systemd-sysv
apt-get -y install locales console-common ntp less openssh-server nano git vim aptitude
